...
class BaseController {
    constructor($scope, $injector) {
        ...
        this.$ds['Items'] = new kendo.data.DataSource({
        ...
        this.$ds['Items'].bind('error', this._$errorHandler.bind(this));

        this.$viewModels['ItemsModel'] = this.$dsService.createPristineModel(this.$ds['Items']);

        this.$customSections = {
            top: 'views.sales.inventory.topSection.html'
        };

        this.$domReady = false;

        this.$components['itemListview'] = {
            widget: null,
            options: {
                ...
                template: (dataItem) => {
                    return kendo.template(angular.element("#itemTemplate").html())(dataItem);
                }
            },
            events: {
                onChange: (e) => {
    ...
    _$destroy() {
        ...
        this.$ds['Items'].unbind('error', this._$errorHandler);
